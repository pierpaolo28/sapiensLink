...
export default function UserProfilePage() {
  ...
  const recentContributions =
    (userProfile?.lists_contributions?.length ?? 0) +
    (userProfile?.ranks_contributions?.length ?? 0);
                  ...
                  <Typography variant="body2" sx={{ my: 1 }}>
                    <Link
                      ...
                      sx={(theme) => ({
                        ...
                        "&:hover": {
                          color:
                            theme.palette.mode === "dark" ? "#fff" : "#101828",
                        },
                      })}
                      href={userProfile.user.social}
                      target="_blank"
                ...
                {listVisibility === "public" && (
                  <>
                    ...
                    {recentContributions > 0 ? (
                      <List>
                        {userProfile?.lists_contributions.map(
                          (contribution, index) => (
                            <Link
                              key={index}
                              href={`/list/${contribution.id}`}
                              color={"text.secondary"}
                              sx={(theme) => ({
                                transition: ".2s",
                                "&:hover": {
                        ...
                        {userProfile?.ranks_contributions.map(
                          (contribution, index) => (
                            <Link
                              key={index}
                              href={`/rank/${contribution.id}`}
                              color={"text.secondary"}
                              sx={(theme) => ({
                                transition: ".2s",
                                "&:hover": {
